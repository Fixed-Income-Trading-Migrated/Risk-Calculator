...
public class RiskCalculator : IRiskCalculator
{
    ...
    public RiskCalculator(ILogger<RiskCalculator> logger, NatsClient natsClient)
    {
        _logger = logger;
        ...
        var task = Task.Run(async () =>
        {
            try
            {
                ...
                await foreach (var msg in  consumer.ConsumeAsync<List<ClientData>>(cancellationToken: cts.Token))
                {
                    ...
                    if (clients == null)
                    {
                        _logger.LogError("Clients not found");
                    ...
                    foreach (var client in clients.Where(client => !_clients.Contains(client)))
                    {
                        ...
                        var task = Task.Run(async () =>
                        {
                            ...
                            catch (OperationCanceledException) { _logger.LogInformation("Stopped subscription: {TopicForClientData}", topicForClientData); }
                        }, ctss.Token);
            ...
            catch (OperationCanceledException) { _logger.LogInformation("Stopped subscription: {TopicForClients}", topicForClients); }
        }, cts.Token);
    ...
    private void SubscribeToAllTargets()
    {
        ...
        var task = Task.Run(async () =>
        {
                    ...
                    if (targets != null)
                    {
                                     ...
                                     !_targetPositions.ContainsKey(target.InstrumentId)))
                        {
                            ...
                            var task = Task.Run(async () =>
                            {
                                ...
                                {
                                    _logger.LogInformation("Stopped subscription: {TopicForTarget}", topicForTarget);
                                }
                            }, ctss.Token);
                            _subscriptionTasks.Add(task);
                        }
            ...
            catch (OperationCanceledException) { _logger.LogInformation("Stopped subscription: {TopicAllTargets}", topicAllTargets); }
        }, cts.Token);
        ...
        var task = Task.Run(async () =>
        {
            try
            {
                var consumer = await _natsClient.CreateJetStreamContext()
                    .GetConsumerAsync("streamOrders", "buyOrderConsumer", cts.Token);
                await foreach (var msg in consumer.ConsumeAsync<Order>(cancellationToken: cts.Token))
                {
                    ...
                    if (msg.Data == null)
                    {
                        _logger.LogError("BuyOrder consumer returned null");
                        await msg.AckAsync(cancellationToken: cts.Token);
                        return;
                    }
                    CheckOrder(msg.Data);
            ...
            {
                _logger.LogInformation("Stopped subscription: {BuyOrder}", topic);
            }
        }, cts.Token);
        _subscriptionTasks.Add(task);
    }
